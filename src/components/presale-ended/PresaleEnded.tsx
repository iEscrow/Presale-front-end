...
const PresaleEnded = () => {
  ...
  const { data: usdValue, isLoading: isUsdValueFetching } = useReadContract({
    ...
    query: {
      enabled: netStatus === 'connected' && userAddress !== undefined
    }
  })
  ...
  return (
    <div className="w-full md:h-[calc(100vh-80px)] flex items-center justify-center">
      <form className="relative w-full sm:max-w-[650px] sm:min-w-[500px]  flex flex-col items-center py-4 px-4 md:px-6 md:py-8 mb-4 rounded-md border-[1px] border-body-text bg-linear-245 from-black/70 from-50% to-logo-grad-blue/30 overflow-hidden z-10">
        <FormTitle title="Buy $ESCROW Token" />
        <p className="text-bg-logo font-light text-xs md:text-sm tracking-tight text-center mt-4">
          The <b>$ESCROW</b> presale has ended! You can connect your wallet and claim your tokens now.
